...
func TestSegment(t *testing.T) {
	...
	// Write initial entry.
	entry1 := createEntry(flagEntryPut, []byte("foo"), []byte("bar"))
	...
	entry2 := createEntry(flagEntryPut, []byte("foo1"), bytes.Repeat([]byte("m"), 3*(1<<20)))
	...
	segment.ForEachEntry(func(e *entry) error {
		switch n {
		case 0:
			if !e.hdr.getFlag().isEntryPut() || !bytes.Equal(entry1.key, e.key) || !bytes.Equal(entry1.value, e.value) {
				t.Fatalf("unexpected entry(0): %q", e)
			}
		case 1:
			if !e.hdr.getFlag().isEntryPut() || !bytes.Equal(entry2.key, e.key) || !bytes.Equal(entry2.value, e.value) {
